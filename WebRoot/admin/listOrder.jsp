			...
			<tbody>
				<c:forEach items="${os}" var="o">
					<tr>
						<td>${o.id}</td>
						<td>${o.statusDesc}</td>
						<td>￥<fmt:formatNumber type="number" value="${o.total}" pattern="0.00" /></td>
						<td>${o.totalNumber}</td>
						<td>${o.user.name}</td>
							...
							<div class="orderPageOrderItem">
								<table width="800px" align="center" class="orderPageOrderItemTable">
									<c:forEach items="${o.orderItems}" var="oi">
										<tr>
											<td><img width="40px" height="40px" src="/pic/tmall_img/productSingle/${oi.product.firstProductImage.id}.jpg" /></td>
											<td><a href="foreproduct?pid=${oi.product.id}">${oi.product.name}</a></td>
											<td><span class="text-muted">${oi.number}个</span></td>
											<td><span class="text-muted">单价：￥${oi.product.promotePrice}</span></td>
										</tr>
									</c:forEach>
								</table>
							</div>
